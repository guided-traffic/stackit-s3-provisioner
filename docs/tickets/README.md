...
slug names the **outcome**, not the topic: `006-recover-from-a-deleted-admin-s3-key.md`,
...
Four live tickets, none approved for implementation. Three carry **2026-09-19** on the file itself,
the date they were raised or last refined; 010 is older than its file — it was open question Q2 of the
...
| [005 — a provisioned bucket that vanished is reported](archive/005-a-provisioned-bucket-that-vanished-is-reported.md) | 2026-09-19 | [ADR 0015](../adr/0015-a-provisioned-bucket-is-never-re-created-implicitly.md) — a bucket deleted behind the operator's back was read as "not there *yet*" and re-created empty, with fresh credentials in the workload Secret and the CR still `Ready`; a provisioned bucket that vanished is now reported and never re-created implicitly, and the ticket's own one-shot re-create trigger was rejected in favour of delete-and-re-apply |
| [007 — hot-reload the StackIT service-account key](archive/007-hot-reload-the-stackit-service-account-key.md) | 2026-09-19 | [ADR 0016](../adr/0016-the-service-account-key-is-reloaded-only-after-it-is-proven.md) — the key was read once at process start and the SDK signed every token refresh from the in-memory copy, so a rotated key on disk changed nothing until somebody restarted the pod, while a revoked one dropped the whole fleet to `Failed` at once; the key is now polled, proven and swapped atomically, and the ticket's fsnotify option was rejected because a watch on the file path follows the old inode through a Secret volume's `..data` swap and silently stops firing |
