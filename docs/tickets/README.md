...
Five live tickets, none approved for implementation. Four carry **2026-09-19** on the file itself, the
...
| [007 — hot-reload the StackIT service-account key](007-hot-reload-the-stackit-service-account-key.md) | Design complete: every listed question answered (Q1–Q17), not approved for implementation | The SA key is read once at process start and the SDK signs every token refresh from the in-memory copy, so a rotated key on disk changes nothing until the pod restarts — while a revoked key is a definitive refusal that drops every `Bucket` to `Failed` at once. The ticket picks up a replaced key without a restart, and refuses an invalid or foreign one without displacing the working client. The guard that makes a foreign key safe has landed ([ADR 0015](../adr/0015-a-provisioned-bucket-is-never-re-created-implicitly.md)), so nothing blocks this any more. |
