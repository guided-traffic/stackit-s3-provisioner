...
| No second change rides along | Narrowing existence checks is [008](008-decide-bucket-existence-with-a-per-bucket-read.md) and key hot-reload was [007](archive/007-hot-reload-the-stackit-service-account-key.md) and has landed ([ADR 0016](../adr/0016-the-service-account-key-is-reloaded-only-after-it-is-proven.md)), which already reshaped that constructor; both touch the same file. This change is a package move and nothing else, so that a failure against the real API has one candidate cause |
