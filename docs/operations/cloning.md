...
| The copy is running but stats are unreachable | `progress`, `rate` and `eta` stop moving; phase stays `Running`; operator log (verbosity 1) `clone stats unavailable` | Nothing to do — stats are best effort (a pod that has not started yet looks exactly like this) and polling continues. The Job's own state, not the stats, decides the outcome. |
