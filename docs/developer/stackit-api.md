...
| `validUntil` | [`stackit.LoadAccount`](../../stackit/client.go), and separately the SDK | `Account.ValidUntil`, nil when the file carries none — a real shape, and the one both key files in this tree have. It feeds `stackit_s3_provisioner_sa_key_valid_until_timestamp_seconds` and the two expiry alerts ([service-account-key-reload.md](service-account-key-reload.md)). Nothing *gates* a load on it, so an expired key file is still not detected before the token exchange fails; it is reported, not enforced |
| `id`, `publicKey`, `active`, `createdAt`, `keyAlgorithm`, `keyOrigin`, `keyType` | parsed by the SDK, acted on by nothing | `clients.ServiceAccountKeyResponse` in `core v0.26.0` declares a typed field for each, so they are unmarshalled; no code path in the SDK or in this repo reads them |

A structurally valid key file for offline tests is built by `buildSAKey` in
[`stackit/keyreload_test.go`](../../stackit/keyreload_test.go), which takes a per-field option for
each shape a test needs; it is the cheapest reference for the exact document.
`writeTestSAKey` in [`stackit/newclient_test.go`](../../stackit/newclient_test.go) is a one-line
wrapper that writes the default one to a temporary file.
...
passes the key **content**, not the path — `config.WithServiceAccountKey(string(content))` in
`newKeyFlowAPIClient` ([`stackit/client.go`](../../stackit/client.go)) — never handles the token and
keeps no token cache of its own; at that lifetime there would be nothing worth caching. Content
rather than path so that the bytes that were hashed and validated are the bytes the SDK receives;
the path is deliberately not passed alongside, because the SDK would fall back to re-reading it if
the content were ever empty ([service-account-key-reload.md](service-account-key-reload.md)).
...
**The key is snapshotted at client construction, and the SDK offers no way to replace it in a live
client.** Verified in `core v0.26.0`: `auth.SetupAuth` → `auth.KeyAuth` takes the key content from
the configuration, unmarshals it, and hands the struct plus the PEM string to
...
`createAccessToken` → `generateSelfSignedJWT`); nothing ever looks at the file again.

That is why a rotation replaces the **whole client** rather than the key: there is nothing smaller
to swap. The operator polls the file itself and, when the content has changed, builds a candidate
client, proves it with one authenticated call and swaps the live one atomically
([ADR 0016](../adr/0016-the-service-account-key-is-reloaded-only-after-it-is-proven.md); the
mechanism is [service-account-key-reload.md](service-account-key-reload.md)). No file watch is
involved — `fsnotify` is still an indirect dependency only, and
[ADR 0016](../adr/0016-the-service-account-key-is-reloaded-only-after-it-is-proven.md) D2 says why
polling was chosen over one. The project and the region remain fixed for the life of the process
([ADR 0005](../adr/0005-the-operator-serves-one-project-in-one-region.md) D1/D2); only the
credential moves.
